...
    // Beispiel: Sommerferien Hessen 2026 (Beginn ca. 06.07.2026)
...
              {Object.keys(PREDEFINED_SUBJECTS).map(s => (
                <TouchableOpacity key={s} onPress={() => {setSelectedSubject(s); setCustomSubject('');}} style={[styles.chip, selectedSubject === s && { backgroundColor: THEME.primary }]}><Text style={[styles.chipText, selectedSubject === s && { color: '#fff' }]}>{s}</Text></TouchableOpacity>
              ))}
            </ScrollView>
            <TextInput 
