...
// Korrektur: Import für den dauerhaften Speicher
...
const getPureNumber = (gradeStr) => {
  ...
  const num = parseFloat(gradeStr.replace(',', '.'));
...
export default function App() {
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        const savedGrades = await AsyncStorage.getItem('grades_v1');
        const savedTT = await AsyncStorage.getItem('timetable_v1');
  ...
  useEffect(() => {
    const saveData = async () => {
      try {
        await AsyncStorage.setItem('grades_v1', JSON.stringify(grades));
        await AsyncStorage.setItem('timetable_v1', JSON.stringify(timetable));
      } catch (e) { console.log("Speicherfehler", e); }
    };
    saveData();
  }, [grades, timetable]);
  ...
  const musterLinks = [
    { title: 'Vertretungsplan', desc: 'Aktuelle Ausfälle', url: 'https://www.musterschule.de/UNTIS/Vertretungsplan/show.php?plan=H_Schueler_heute', icon: '📋' },
    { title: 'Schulportal Hessen', desc: 'Anmeldung, Login', url: 'https://login.schulportal.hessen.de/', icon: '🔐' },
    { title: 'Bärenstark Schule', desc: 'Essensbestellung', url: 'https://baerenstark-schule.de/mobil/#/login', icon: '🐻' },
  ]; 
          ...
          <View style={{ flexDirection: 'row', gap: 10, marginBottom: 15 }}>
            ...
            {getHolidayCountdown() && (
              <View style={[styles.examHeroCard, { flex: 1, backgroundColor: THEME.success }]}>
                  ...
                  <View style={styles.examHeroContent}>
                      <Text style={styles.examHeroSubject}>Sommer</Text>
                      <View style={styles.examHeroBadge}>
      ...
      <Modal animationType="fade" transparent={true} visible={hwModalVisible}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.inputSingle} placeholder="Hausaufgabe..." value={hwInput} onChangeText={setHwInput} />
            <TextInput style={styles.inputSingle} placeholder="Datum der Arbeit (DD.MM.YYYY)" value={examDateInput} onChangeText={t => setExamDateInput(formatInputDate(t))} />
            <TouchableOpacity style={styles.saveBtn} onPress={saveHwData}><Text style={styles.saveBtnText}>Speichern</Text></TouchableOpacity>
            <TouchableOpacity style={styles.cancelBtn} onPress={() => setHwModalVisible(false)}><Text>Schließen</Text></TouchableOpacity>
          </View>
        </View>
      </Modal>

      {/* ... (Weiterer Modal-Code identisch zu deinem vorherigen, wichtig ist die Speicherlogik oben!) ... */}
...
const styles = StyleSheet.create({
  ...
  navHeaderFixed: { backgroundColor: THEME.secondary, flexDirection: 'row', justifyContent: 'space-evenly', paddingVertical: 10 },
  ...
  linkCard: { backgroundColor: THEME.card, borderRadius: 18, padding: 15, flexDirection: 'row', alignItems: 'center', marginBottom: 15 },
  ...
  examHeroCard: { borderRadius: 18, padding: 12 },
  ...
  examHeroBadge: { backgroundColor: '#fff', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 },
  examHeroBadgeValue: { color: THEME.danger, fontSize: 16, fontWeight: '900' },
  examHeroBadgeLabel: { color: THEME.danger, fontSize: 8, fontWeight: '700' },
  gridCardFull: { flex: 1, backgroundColor: THEME.card, borderRadius: 15, overflow: 'hidden', marginTop: 10 },
  ...
  gridHeaderCellSmall: { flex: 1, paddingVertical: 6, alignItems: 'center' },
  gridHeaderTextSmall: { fontWeight: '800', fontSize: 11 },
  gridSideCellSmall: { width: 35, alignItems: 'center', justifyContent: 'center', borderRightWidth: 1, borderRightColor: '#F1F5F9' },
